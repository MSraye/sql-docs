...
- Fix an issue that sometimes SSIS Build Task of version 1.* failed to build projects/packages with protection level EncryptSensitiveWithPassword or EncryptAllWithPassword with error "Specified initialization vector (IV) does not match the block size for this algorithm.".
